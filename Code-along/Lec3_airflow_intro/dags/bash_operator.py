...
with DAG(dag_id="joke_DAG", start_date=datetime(2023, 6, 1)):
    ...
    download_joke = BashOperator(task_id="random_joke", bash_command=f"curl -o {data_lake_path}/joke{time_variable}.json https://official-joke-api.appspot.com/random_joke")

    download_norris_joke = BashOperator(
        task_id = "chuck_norris_joke",
        bash_command= f"curl -H 'X-Api-Key: {API_NINJA_KEY}' -o {data_lake_path}/norris{time_variable}.json https://api.api-ninjas.com/v1/chucknorris"
    )
