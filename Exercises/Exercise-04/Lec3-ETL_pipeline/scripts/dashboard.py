...
def user_map(user_lat = 50, user_lon = 50):
    ...
    fig.update_layout(
        ...
        mapbox=dict(center=dict(lat=user_lat, lon=user_lon)),
    )
    return fig
...
def user_card(user_id):
    ...
    user_map(user['latitude'], user['longitude'])
...
@app.callback(Output("user-map", "figure"), Input("users-dropdown", "value"))
def update_map(user_id):

    user = df.loc[user_id]
    return user_map(user['latitude'], user['longitude'])
